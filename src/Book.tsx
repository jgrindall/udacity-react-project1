import * as React from 'react';
import {IBookProps, Shelf, IOption} from "./types";
import {ChangeEvent} from "react";
import {getLabel} from "./Labels";

const shelves = [
    Shelf.SHELF_CURRENTLY_READING,
    Shelf.SHELF_WANT_TO_READ,
    Shelf.SHELF_READ
];

const EMPTY_THUMBNAIL = "data:image/jpeg;base64,";

class Book extends React.Component<IBookProps, {}> {

    onSelect(event:ChangeEvent){
        this.props.onMove(this.props.book, event.target.value);
    }

    render(){
        const moveOptions = shelves.map( (shelf:Shelf, i:number)=>{
            const isCurrentShelf = (this.props.book.shelf === shelf);
            return {
                value:shelf,
                label: getLabel(shelf),
                disabled: isCurrentShelf
            };
        });

        const options:IOption[] = [
            {
                value:"move",
                label:"Move to...",
                disabled:true
            },
            ...moveOptions,
            {
                value:"none",
                label:"None",
                disabled: !shelves.includes(this.props.book.shelf)
            }
        ];

        const makeOptionElement = (option:IOption)=>{
            const prefix = (this.props.book.shelf === option.value ? "* " : "");
            return <option
                key={option.value}
                value={option.value}
                disabled={option.disabled}
            >
                {prefix + option.label}
            </option>;
        };

        const value = this.props.book.shelf || "none";

        const bg = this.props.book.imageLinks?.smallThumbnail || EMPTY_THUMBNAIL;

        return (
            <li>
                <div className="book">
                    <div className="book-top">
                        <div className="book-cover" style={{
                            backgroundImage: `url(${bg})`
                        }}></div>
                        <div className={"book-shelf-changer" + (shelves.includes(this.props.book.shelf) ? ' exists' : '')}>
                            <select value={value} onChange={(e)=>this.onSelect(e)}>
                                {options.map(makeOptionElement)}
                            </select>
                        </div>
                    </div>
                    <div className="book-title">
                        {this.props.book.title}
                    </div>
                    <div className="book-authors">
                        {(this.props.book.authors || []).join(', ')}
                    </div>
                </div>
            </li>
        );
    }
}

export default Book;
